    ...
    <>
      <div className="flex flex-col gap-4">
        <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
          {agents.length} agent{agents.length !== 1 ? 's' : ''} available — click a card to inspect its steps and prompts
        </p>
          ...
          {sorted.map((agent) => {
            ...
            return (
              <button
                ...
                className="flex flex-col gap-2 rounded-lg p-4 text-left transition-colors"
                style={{
                  backgroundColor: 'var(--bg-surface)',
                  border: `1px solid ${isDefault ? 'var(--purple-600)' : 'var(--border-custom)'}`,
                }}
                onMouseEnter={(e) => {
                  (e.currentTarget as HTMLButtonElement).style.borderColor = 'var(--purple-600)'
                }}
                onMouseLeave={(e) => {
                  (e.currentTarget as HTMLButtonElement).style.borderColor =
                    isDefault ? 'var(--purple-600)' : 'var(--border-custom)'
                }}
              >
