...
import type { SuccessfulPattern } from '@/types'
import { formatAgentType } from '@/lib/utils'

interface Props {
  data: SuccessfulPattern[]
}
...
export function SuccessfulPatternsTable({ data }: Props) {
  if (data.length === 0) {
    return (
      <div
      ...
      >
        No successful patterns yet. Successful runs will be recorded here automatically.
      </div>
    )
  }
          ...
          {data.map((p, idx) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-xs font-mono" style={{ color: 'var(--text-primary)' }}>
                {p.avgLatency != null ? `${p.avgLatency.toLocaleString()} ms` : <Muted />}
              </td>
              <td className="px-4 py-3 text-xs font-mono" style={{ color: 'var(--text-primary)' }}>
                {p.avgTokens != null ? p.avgTokens.toLocaleString() : <Muted />}
              </td>
